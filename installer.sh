...
if docker compose version >/dev/null 2>&1; then
    ...
    echo -e "${RED}❌ No Docker Compose found. Please install it.${NC}"
    exit 1
fi
...
echo -e "\n📂 Preparing storage directories at: ${BLUE}$STORAGE_PATH${NC}"
...
if ! command -v lsusb &> /dev/null; then
    echo -e "${YELLOW}⚠️  'lsusb' command not found.${NC}"
    ...
    if [[ "$INSTALL_LSUSB" =~ ^[Yy]$ ]]; then
        ...
        if [ $? -ne 0 ]; then
            echo -e "${RED}❌ Installation failed. Skipping USB setup.${NC}"
        fi
    fi
fi
        ...
        if [ -n "$USB_SELECTION" ]; then
            ...
            for index in $USB_SELECTION; do
                ...
                if [ -n "$RAW_LINE" ]; then
                    ...
                    echo -e "✅ Adding: ${BLUE}$USB_PATH${NC} ($NAME)"
...
if [ ! -f "$FILENAME" ]; then
    echo -e "⬇️  Downloading installer ($VERSION)..."
